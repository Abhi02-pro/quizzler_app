...
class QuizBrain {
  ...
  String getQuestions() {
    return _question[_questionNum % _question.length].question;
  }

  void checkAnswer(bool userAnswer) {
    if (userAnswer == _question[_questionNum % _question.length].answer) {
  ...
  List answerHistory() {
    if (scoreKeeper.length % _question.length == 0) {
      // Alert(context: context, title: "HURREY !", desc: "You Finished The Quiz.")
      //     .show();
      scoreKeeper = [];
    } else {
  ...
  bool quizFinished() {
    if (_questionNum == _question.length) {
      _questionNum = 0;
      return true;
    } else {
      return false;
